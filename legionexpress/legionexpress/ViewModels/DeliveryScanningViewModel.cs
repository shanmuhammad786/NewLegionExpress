...
{
    public class DeliveryScanningViewModel : BaseViewModel, INotifyPropertyChanged, IBarcodeCaptureListener
    {
        ...
        public int CollectionScannedCount
        {
            get
            {
                return _collectionScannedCount;
            }
            set
            {
        ...
        public DeliveryScanningViewModel(INavigation navigation)
        {
            this.Navigation = navigation;
            _service = new ShipmentService();
            AmendmendVisibility = false;
            Preferences.Remove("CollectionScannedCount");
            CollectionScannedCount = 0;
            Utilty.CollectionScanCount = 0;
        ...
        public ICommand DeliveredCommand => new Command(Delivered);
        public ICommand AttemptedCommand => new Command(Attempted);
        public ICommand CheckAddressCommand => new Command(CheckAddress);
        #endregion
        ...
        {
            await PopupNavigation.Instance.PushAsync(new Delivered() { BindingContext = this});
        }
        private async void Attempted()
        {
            await PopupNavigation.Instance.PushAsync(new Attempted() { BindingContext = this });
        }
        private async void CheckAddress()
        {
            await PopupNavigation.Instance.PushAsync(new CheckAddress() { BindingContext = this });
        }

        public void InitializeScanner()
        {
            // this.BarcodeCapture = ScannerModel.Instance.BarcodeCapture;
            // Register self as a listener to get informed whenever a new barcode got recognized.
            //if (this.BarcodeCapture == null)
            //{
            //    Camera = ScannerModel.Instance.CurrentCamera;

            //    BarcodeCaptureSettings = ScannerModel.Instance.BarcodeCaptureSettings;
            //    DataCaptureContext = ScannerModel.Instance.DataCaptureContext;
            //    //DataCaptureView dataCaptureView { get; private set; } = ScannerModel.Instance.DataCaptureContext

            //    BarcodeCapture = ScannerModel.Instance.BarcodeCapture;
        ...
        private async void GetScannedShipment(string barCode)
        {
            ...
            try
            {
                ...
                Analytics.TrackEvent("Collection Scanning is started", scanStartProperties);
                IsLoading = true;
                ...
                if (shipment != null && shipment.hasError == false)
                {
                    ...
                    CollectionScannedCount = newCount;
                    Utilty.CollectionScanCount = newCount;

                    //Preferences.Set("CollectionScannedCount", newCount.ToString());
                ...
                Analytics.TrackEvent("Collection Scanning is ended", scanEndProperties);
            }
            catch (Exception ex)
            {
                Dictionary<string, string> scanStartProperties = new Dictionary<string, string>();
